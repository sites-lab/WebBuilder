...
function dmOnLogin(u) {
  ...
  _syncAuthJs();
  if (_panelOpen) setTimeout(() => document.getElementById('wbp-msg-txt')?.focus(), 200);
}
...
async function wbpMsgAuth() {
  ...
  try {
    ...
    // onAuthStateChanged fires → dmOnLogin handles the UI switch automatically
  } catch(e) {
    const msgs = {
...
async function wbpMsgResetPwd() {
  ...
  } catch(e) { dmShowErr('Could not send reset email. Check the address.'); }
}
...
// ── Panel open / close ─────────────────────────────────────────────────
// The FAB button in index.html calls wbpMsgDirectOpen() defined inline there.
// That function opens the panel with NO redirect — ever.
// If logged in → dmOnLogin() shows chat. If not → sign-in form shows inside panel.
...
// ── Expose globals ─────────────────────────────────────────────────────
// window._dmUser and window._dmOnLogin are read by wbpMsgDirectOpen in index.html
...
Object.defineProperty(window, '_dmUser', { get: () => _dmUser, configurable: true });
